...
function MoodSelector({ selectedMood, onMoodSelect }) {
    return (<div className="flex justify-center space-x-4">
      {moods.map((mood) => (<button key={mood.value} onClick={() => onMoodSelect(mood.value)} className={`mood-emoji w-16 h-16 bg-gradient-to-br ${mood.color} rounded-2xl flex items-center justify-center text-2xl transition-all duration-300 ease-out hover:scale-110 ${selectedMood === mood.value ? 'selected scale-120 ring-4 ring-[var(--mindtune-primary)]/30' : ''}`}>
          {mood.emoji}
        </button>))}
    </div>);
}
